...
def collect_analytics_for_sis_course_id(course_id, time_period):
    ...
    for user in users:
        ...
        try:
            assignment_data = (
                Analytics()
                .get_student_assignments_for_sis_course_id_and_canvas_user_id(
                    course_id, person_id))
            for assignment in assignment_data:
                ...
                if "submission" in assignment:
                    ...
                    return_values.append({
                        "login_name": user.login_id,
                        "type": ("Assignment submission date - %s" %
                                 assignment["title"]),
                    ...
                    if get_days_late:
                        try:
                            ...
                            # Either the day turned in, or today
                            date_against = None
                            if assignment["status"] == "late":
                                date_against = parser.parse(
                    ...
                    # This is for scores.  I *think* each assignment is
                    # points-based, with different displays. This tries to turn
                    # them into a percentage, so there's some normalization.
                    # If that fails, it'll return a raw score instead, which
                    # hopefully covers any other bad assumptions made.
                    try:
                        ...
                        assignment_grade_data = {
                            "login_name": user.login_id,
                            "type": ("Assignment - %s - Points Possible" %
                                     assignment["title"]),
                            "value": possible
                        }
                        ...
                        if possible is None or possible == 0:
                            ...
                            assignment_grade_data = {
                                "login_name": user.login_id,
                                "type": ("Assignment - %s - Percentage Score" %
                                         assignment["title"]),
                                "value": (
                                    (float(score) / float(possible)) * 100.0)
                            }
                        return_values.append(assignment_grade_data)
                        ...
                        print("Score: {}, Possible: {}"
                              .format(score, possible))
                        print("Error: {}".format(ex))
                        assignment_grade_data = {
                            "login_name": user.login_id,
                            "type": ("Assignment - %s - ERROR" %
                                     assignment["title"]),
                            "value": str(ex)
                        }
                        return_values.append(assignment_grade_data)

...
def collect_analytics_for_sis_course_id(course_id, time_period):
    ...
    for assignment in assignments:
        per_user_counts = {}
        url = ("/api/v1/courses/sis_course_id:%s/assignments/%s/submissions/" %
        ...
        for submission in submissions:
            ...
            per_user_counts[person_map[user_id]] = \
                per_user_counts[person_map[user_id]] + 1

        for user in per_user_counts:
            return_values.append({
                "login_name": user,
                "type": ("Assignment submission count for %s" %
                         (assignment.name)),
